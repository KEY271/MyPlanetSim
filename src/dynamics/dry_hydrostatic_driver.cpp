...
DryHydrostaticRhs DryHydrostaticDriver::rhs(const DryHydrostaticState& s) const {
  ...
  for (const auto& e : grid_.edges()) {
    ...
    for (std::size_t k = 0; k < K; ++k) {
      auto ln = dry_hydrostatic_offset(l, k, K);
      auto rn = dry_hydrostatic_offset(r, k, K);
      DryHydrostaticPrimitive lp{d.air_mass_kg_m2[ln], d.velocity_m_s[ln],
                                 d.potential_temperature_k[ln],
                                 d.tracer_mixing_ratio[ln], d.temperature_k[ln]},
          rp{d.air_mass_kg_m2[rn], d.velocity_m_s[rn], d.potential_temperature_k[rn],
             d.tracer_mixing_ratio[rn], d.temperature_k[rn]};
      auto f = rusanov_dry_hydrostatic_flux(lp, rp, basis,
...
void DryHydrostaticDriver::advance(DryHydrostaticState& s, const Real end,
  ...
  while (s.time_s < end) {
    ...
    auto r = rhs(s);
    auto dt = std::min(
        {config_.run.time_step_s, r.horizontal_stable_time_step_s, end - s.time_s});
    if (!(dt > 0)) throw std::runtime_error("dry driver stable time step is invalid");
    for (std::size_t c = 0; c < s.surface_pressure_pa.size(); ++c)
      s.surface_pressure_pa[c] += dt * r.surface_pressure_pa_s[c];
    for (std::size_t n = 0; n < s.horizontal_momentum_mass_kg_m_s.size(); ++n) {
      s.horizontal_momentum_mass_kg_m_s[n] =
          s.horizontal_momentum_mass_kg_m_s[n] + dt * r.tendency.momentum[n];
      auto c = n / coordinate_.levels();
      s.horizontal_momentum_mass_kg_m_s[n] = project_tangent(
          s.horizontal_momentum_mass_kg_m_s[n], grid_.cells()[c].center);
      s.potential_temperature_mass_k_kg_m2[n] +=
          dt * r.tendency.potential_temperature_mass[n];
      s.tracer_mass_kg_m2[n] += dt * r.tendency.tracer_mass[n];
    }
    s.time_s += dt;
    ++s.step;
    auto d = diagnose(s);
    std::vector<Vec3> centres;
    for (auto& cell : grid_.cells()) centres.push_back(cell.center);
    validate_dry_hydrostatic_state(s, d, centres,
                                   config_.vertical.minimum_surface_pressure_pa,
                                   config_.vertical.maximum_surface_pressure_pa,
