...
DryHydrostaticState initialize_dry_hydrostatic_benchmark(
  ...
  for (std::size_t cell = 0; cell < cells; ++cell) {
    ...
    if (config.dry_hydrostatic.test_case == DryHydrostaticTestCase::kLinearWave) {
      state.surface_pressure_pa[cell] *= 1.0 + 1.0e-5 * std::cos(longitude) *
                                                  std::cos(latitude);
    }
    ...
    for (std::size_t level = 0; level < levels; ++level) {
      ...
      const Real sigma = (static_cast<Real>(level) + 0.5) /
      ...
      switch (config.dry_hydrostatic.test_case) {
        ...
        case DryHydrostaticTestCase::kDcmipHadley:
          velocity = 8.0 * std::sin(2.0 * latitude) *
                     project_tangent(Vec3{0, 0, 1}, position);
        ...
        case DryHydrostaticTestCase::kUmjs14Baroclinic:
          velocity = 35.0 * std::cos(latitude) *
                     normalize(cross(Vec3{0, 0, 1}, position));
          break;
        default:
          break;
      }
      const Real theta = config.vertical.initial_temperature_k /
      ...
      state.tracer_mass_kg_m2[offset] =
          geometry.air_mass_kg_m2[level] * tracer;
    }
  }
  return state;
}
