...
LatLonTerrain parse_latlon_csv(const std::string& bytes) {
  ...
  std::string line;
  if (!std::getline(input, line)) throw std::runtime_error("terrain CSV is empty");
  ...
  while (std::getline(input, line)) {
    ...
    const Real longitude = parse_csv_real(std::string_view(line).substr(0, first));
    const Real latitude =
        parse_csv_real(std::string_view(line).substr(first + 1, second - first - 1));
...
std::vector<Real> interpolate_latlon(const LatLonTerrain& terrain,
  ...
  for (std::size_t cell = 0; cell < grid.cell_count(); ++cell) {
    ...
    Real lon = std::atan2(position.y, position.x) * 180.0 / std::numbers::pi_v<Real>;
    ...
    const Real wy =
        lat0 == lat1 ? 0.0 : (lat - latitude[lat0]) / (latitude[lat1] - latitude[lat0]);
...
std::vector<Real> smooth_surface_geopotential(const CubedSphereGrid& grid,
  ...
  for (Index pass = 0; pass < passes; ++pass) {
    ...
    for (const auto& edge : grid.edges()) {
      ...
      const Real capacity =
          0.5 *
          std::min(grid.cells()[left].area_m2 /
...
SurfaceOrography::SurfaceOrography(std::vector<Real> surface_geopotential_m2_s2,
...
Real dcmip_2_0_0_surface_height_m(const Vec3 position) {
  ...
  const Real oscillation = std::cos(std::numbers::pi_v<Real> * distance / half_width);
...
Real williamson5_surface_height_m(const Vec3 position) {
  ...
  const Real distance = std::hypot(longitude_distance, latitude - centre_latitude);
...
Real jw06_surface_geopotential_m2_s2(const Vec3 position,
  ...
  const Real eta_v = (1.0 - kJw06Eta0) * 0.5 * std::numbers::pi_v<Real>;
  ...
  return kJw06U0Mps * vertical *
         (kJw06U0Mps * vertical * wind_shape +
...
SurfaceOrography make_surface_orography(const OrographyParameters& parameters,
  ...
  if (parameters.kind == OrographyKind::kFlat)
    return SurfaceOrography(std::vector<Real>(grid.cell_count(), 0.0), "analytic:flat");
  if (parameters.kind == OrographyKind::kDcmip200) {
    std::vector<Real> geopotential(grid.cell_count());
    for (std::size_t cell = 0; cell < grid.cell_count(); ++cell)
      geopotential[cell] =
          gravity_m_s2 * dcmip_2_0_0_surface_height_m(grid.cells()[cell].center);
  ...
  if (parameters.kind == OrographyKind::kWilliamson5) {
    ...
    for (std::size_t cell = 0; cell < grid.cell_count(); ++cell)
      geopotential[cell] =
          gravity_m_s2 * williamson5_surface_height_m(grid.cells()[cell].center);
  ...
  if (parameters.kind == OrographyKind::kLinearBell) {
    ...
    for (std::size_t cell = 0; cell < grid.cell_count(); ++cell)
      geopotential[cell] =
          gravity_m_s2 * linear_bell_surface_height_m(grid.cells()[cell].center);
  ...
  if (parameters.kind == OrographyKind::kJw06) {
    ...
    for (std::size_t cell = 0; cell < grid.cell_count(); ++cell)
      geopotential[cell] =
          jw06_surface_geopotential_m2_s2(grid.cells()[cell].center, planet);
  ...
  if (parameters.kind == OrographyKind::kLatLonCsv) {
    ...
    if (!input)
      throw std::runtime_error("unable to open terrain CSV: " + path.string());
    ...
    auto geopotential = interpolate_latlon(parse_latlon_csv(bytes), grid, gravity_m_s2);
    geopotential =
        smooth_surface_geopotential(grid, geopotential, parameters.smoothing_passes);
