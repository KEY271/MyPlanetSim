#!/usr/bin/env python3
"""Summarize benchmark_radiation's accepted-step and sampled climate diagnostics."""
import argparse
import csv
import json
import math
...
def main():
    ...
    parser.add_argument('--radius-m', type=float, default=6371220.0, help='planet radius used for flux normalization')
    ...
    for name, start, end in [('first_100_days', 0, 100), ('last_100_days', final_day-100, final_day),
        ...
        window = {'accepted_steps': len(selected),
                  ...
                  'linear_iterations_per_step': statistics.mean(row['linear_iterations'] for row in selected)}
        for key in ('cfl_retries', 'invariant_retries', 'solver_retries', 'toa_net_j',
                    'surface_storage_j', 'dry_thermal_j', 'drag_j', 'diffusion_j',
                    'surface_integration_residual_j'):
            window[key] = sum(row[key] for row in selected)
        duration_s = sum(row['dt_s'] for row in selected)
        window['integrated_toa_net_w_m2'] = window['toa_net_j'] / (duration_s * area)
        window['integrated_surface_storage_w_m2'] = window['surface_storage_j'] / (duration_s * area)
        for key in ('mean_temperature_k', 'mean_surface_temperature_k', 'maximum_wind_m_s',
                    'toa_net_w_m2', 'surface_storage_w_m2', 'unstable_fraction', 'minimum_dtheta_dz_k_m'):
            # Integrate the piecewise-linear samples using their actual times.
            ...
            window[key] = {'time_mean': mean, 'minimum': min(row[key] for row in samples),
                           'maximum': max(row[key] for row in samples)}
        result['windows'][name] = window
    result['maximum_sampled_mass_relative_drift'] = max(abs(row['mass_relative_drift']) for row in climate)
    result['final_energy_residual_j'] = climate[-1]['energy_residual_j']
    result['maximum_interface_residual_power_w'] = max(abs(row['interface_residual_j']) / row['dt_s'] for row in steps)
