...
export function ColumnProfile({ frame, field, cell, level, onSelectLevel }: ColumnProfileProps) {
  ...
  const { points, ticks, minimum, maximum } = profileGeometry(column);
  ...
  return <article className="view-panel column-profile">
    ...
    <div className="column-profile-body">
      <svg viewBox={`0 0 ${profileSize.width} ${profileSize.height}`} role="img" preserveAspectRatio="xMidYMid meet"
        ...
        {ticks.map((tick) => <g key={tick.index}>
          <line className="axis" x1={profilePlot.left - 4} x2={profilePlot.left} y1={tick.y} y2={tick.y} />
          <text className="tick" x={profilePlot.left - 7} y={tick.y + 3} textAnchor="end">{Math.round(tick.pressurePa / 100)}</text>
        </g>)}
        ...
        <text className="tick" x={centreX} y={profileSize.height - 4} textAnchor="middle">{field.label} ({field.unit})</text>
        ...
        {points.map((point) => <circle key={point.index} cx={point.x} cy={point.y} r={point.index === level ? 5 : 2.6}
          className={point.index === level ? "profile-point selected" : "profile-point"}
          onClick={() => onSelectLevel(point.index)}>
          <title>{`Level ${point.index}: ${formatValue(column.values[point.index])} ${field.unit} at ${formatValue(column.pressurePa[point.index])} Pa`}</title>
        </circle>)}
      </svg>
