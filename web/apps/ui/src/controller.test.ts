...
describe("simulation controller", () => {
  it("runs a mock request and stores the authoritative frame", async () => {
    ...
    expect(controller.value.frames.every((frame) => frame.grid.cellsPerPanel === 2)).toBe(true);
    ...
    expect(Math.max(...controller.value.frames.at(-1)!.fields.find((field) => field.id === "speed_m_s")!.values)).toBeGreaterThan(0);
    expect([...controller.value.frames.at(-1)!.fields.find((field) => field.id === "depth_anomaly_m")!.values].some((value) => Math.abs(value) > 0)).toBe(true);
  });
});
