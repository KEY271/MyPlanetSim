...
[[nodiscard]] mps::ExperimentConfig config() {
  ...
  value.run = {
      .start_time_s = 0.0, .end_time_s = 1.0, .time_step_s = 0.1, .random_seed = 1};
...
MPS_TEST_CASE("preserved Gaussian edit has zero global mass change") {
  ...
  const auto edit =
      mps::InitialConditionEditV1{.center_unit = mps::normalize({1.0, 2.0, 3.0}),
                                  .amplitude_m = 1.0,
                                  .sigma_rad = 0.2,
                                  .mass_policy = mps::MassPolicy::kPreserveGlobal};
  const auto diagnostics =
      mps::apply_initial_condition_edits(grid, state, {&edit, 1}, 0.1);
...
MPS_TEST_CASE("allow-change Gaussian edit records added volume") {
  ...
  const auto edit =
      mps::InitialConditionEditV1{.center_unit = {0.0, 0.0, 1.0},
                                  .amplitude_m = 0.5,
                                  .sigma_rad = 0.3,
...
MPS_TEST_CASE("Gaussian edit keeps momentum tangent and rejects invalid edit") {
  ...
  MPS_CHECK_THROWS_AS(
      mps::apply_initial_condition_edits(grid, state, {&invalid, 1}, 0.1),
