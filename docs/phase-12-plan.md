...
状態: 調査に基づく実装提案。2026-09-07、既存コード `7726bdc` を確認。
本書の設定名・係数・受け入れ閾値は提案値であり、実装済み機能や検証結果ではない。
...
- [ ] P12.01--P12.09 の実装・独立 column・結合・restart ゲートを通過した。
- [ ] 対流が enthalpy を保存して安定性を満たし、E_dry との差を説明・計測できる。
- [ ] 境界層の共有 flux、tracer 保存、地表 impulse、散逸熱が局所収支に対応する。
- [ ] 旧顕熱・Rayleigh drag の二重計上がなく、無効時に旧結果・fingerprint が維持される。
- [ ] 地表付近の解像範囲、結合の一次精度、30分刻み可否、係数感度が明記されている。
- [ ] 全球行列・30日比較・1200日 pilot と全回帰、GCC/Clang、ASan/UBSan、format-check を実施した。
- [ ] `docs/validation/phase-12.md` に再現コマンド、版付き参照 fixture、失敗例と未解決事項を残した。
