...
[Phase 13 実装計画](phase-13-plan.md) に定める。P13.01--P13.10 は実装・検証済みで、
[検証報告](validation/phase-13.md) の全球比較・係数感度・1200日 pilot を完走した。
数値的な pilot gate は完了したが、気候適合性または厳密な気候平衡は宣言しない。
...
[Phase 14 実装計画](phase-14-plan.md) に定める（P14.01--P14.12 実装済み。統合候補
`configs/phase14_moist_candidate.cfg` の1日・30日比較で旧 serial 比 1.86--1.93倍。
気候統計としての採否は300日以上の block 平均に持ち越し、foreground 作業として残る）。
...
- column workspace と保存的な edge flux/gather を整備し、column physics と RHS/fast operator 全域へ OpenMP を広げる
- reconstruction と flux の融合、blocked/AoSoA 配置、modal transform の batch/SIMD 化、tile 処理を比較する
- 同一精度・同一モデル期間での性能、retry、過程収支、restart、30日・1200日統計を検証する

完了ゲート: 各候補の採否と実測を記録し、採用した構成が旧経路の保存・正値性・統計の gate を満たす。
...
[Phase 15 実装計画](phase-15-plan.md) に定める（P15.01--P15.08 実装済み）。
...
- MPI、分散 halo exchange、並列 I/O、accelerator backend
...
Phase 15 の整理は完了した。次の物理拡張へ進む前にも、Phase 14 の長期統計・性能の
未完了項目は、その検証記録に引き続き残す。
