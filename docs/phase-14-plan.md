...
状態: P14.01–P14.12 実装済み（2026-09-09）。ARK2 IMEX は自然な構成で ICI2 より高コストのため不採用。
P14.12 で統合候補 `configs/phase14_moist_candidate.cfg` を登録し、N=6/K=20 の1日比較で
旧 serial 比 1.86–1.89倍（ICI2 併用で 2.03–2.07倍）を測定した。
30日比較では候補の差が微小摂動 baseline 3本の内部変動の内側にあり、採否は300日以上の
block 平均に持ち越した。CAPE/降水tail・地形ケース・高解像度統合速度も未了。
